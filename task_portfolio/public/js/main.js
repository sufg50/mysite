...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "default", function() { return BackGroundMap; });
function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }
...
var BackGroundMap = /*#__PURE__*/function () {
  ...
  _createClass(BackGroundMap, [{
    ...
    key: "createTexture_from_currentArea",
    value: function createTexture_from_currentArea(offsetX, offsetY, game, player, textArea) {
      //stageのchildが入ったままなのでリセットする。
      game.app.stage.removeChildren(); //フェードアウトするときコンテナに入りまくってるからリセットしておく
      ...
      for (var i = 0; i < this.currentArea.length; i++) {
        for (var j = 0; j < this.currentArea[0].length; j++) {
          //PIXI.utils.TextureCache["we1"]
          switch (this.currentArea[i][j]) {
            ...
            default:
              ...
              game.MapChipKansokujoWeather(this.currentArea[i][j], i, j, offsetX, offsetY, 3, player); // console.log("game.currentAreaにその他の値が入っています:"+this.currentArea[i][j]);
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "default", function() { return EnemyMap; });
...
var EnemyMap = /*#__PURE__*/function () {
  function EnemyMap(x, y) {
    _classCallCheck(this, EnemyMap);
    ...
    this.enemyContainer.sortableChildren = true; // this.enemyAnime=[];
  ...
  _createClass(EnemyMap, [{
    ...
    key: "deleteEnemy",
    value: function deleteEnemy(mapAry, enemys) {
      this.enemyMap = new Array(mapAry.length); //移動のための2マス文多めにとる

      for (var y = 0; y < mapAry.length; y++) {
    ...
    key: "setEnemyMap",
    value: function setEnemyMap(mapAry, enemys) {
      // 2次元配列の初期化
      this.enemyMap = new Array(mapAry.length); //移動のための2マス文多めにとる
      ...
      for (var mi = 0; mi < mapAry.length; mi++) {
        //マップのx1,x2がいくつあるか
        for (var mj = 0; mj < mapAry[0].length; mj++) {
          //y0,y1がいくつあるか  
          // for(let i=0;i<8;i++){
      ...
      for (var _i = 0; _i < 8; _i++) {
        if (mapAry[this.enemysX[_i]][this.enemysY[_i]] != "9" && mapAry[this.enemysX[_i]][this.enemysY[_i]] != "p") {
          this.enemyMap[this.enemysX[_i]][this.enemysY[_i]] = "enemy1";
          enemys.push(new EnemyMap(this.enemysX[_i], this.enemysY[_i]));
        }
      }
    } // setEnemyMap終わり
    ...
    key: "enemyAnimation2",
    value: function enemyAnimation2(direction, enemy, game) {
      game.app.stage.removeChild(enemy.enemyAnime);
      var enemyAnimeTextures = [];
      ...
      if (enemy.animeDir != direction) {
        // 前と移動方向が異なるとき
        console.log("前と移動方向が異なるとき");
        enemy.animeDir = direction; // アニメーションのスプライトを再セット
      ...
      enemy.enemyAnime.y = enemy.localX * 48 - 48; // enemy.beforeDirX = enemy.dirX;
      // enemy.beforeDirY = enemy.dirY;

      console.log("移動した鳥の座標x:" + enemy.localX);
      console.log("移動した鳥の座標y:" + enemy.localY);
      game.app.stage.addChild(enemy.enemyAnime); // this.dirMoveAnimetion(enemy.enemyAnime,offsetX,offsetY);
    }
  }, {
    key: "enemyAnimation",
    value: function enemyAnimation(direction, enemy, game) {
      game.app.stage.removeChild(enemy.enemyAnime); // console.log("エネミーワールド座標@enemyAnimation:"+enemy.x+","+enemy.y);
      ...
      enemy.beforeDirY = enemy.dirY; // console.log("移動した鳥の座標x:"+enemy.dirX);
    ...
    key: "setRouteMap",
    value: function setRouteMap(currentArea, localX, localY) {
      ...
      for (var mi = 0; mi < currentArea.length; mi++) {
        //マップのx1,x2がいくつあるか
        for (var mj = 0; mj < currentArea[0].length; mj++) {
          //y0,y1がいくつあるか
          switch (currentArea[mi][mj]) {
            case "enemy1":
              //localX,localYは1個のエネミーのローカル座標が渡される
              enemyX.push(localX);
              enemyY.push(localY);
      ...
      var count = 1; //currenetAreaにエネミーがいてプレイヤと位置が重なってないとき
    ...
    key: "loopRuttingMap",
    value: function loopRuttingMap(pathX, pathY, count, player) {
      var minX = pathX - 1;
      var maxX = pathX + 1;
      var minY = pathY - 1;
      var maxY = pathY + 1;
      var pathsX = new Array();
      var pathsY = new Array(); // ここからルートマッピングを始める。ダイクストラ法を使う
      ...
      if (this.loopRuttingFrag) {
        if (minY >= 0) {
          //左
          if (this.routeMap[pathX][pathY - 1] == 0 || count < this.routeMap[pathX][pathY - 1] || this.routeMap[pathX][pathY - 1] == "p") {
        ...
        if (minX >= 0) {
          //上
          if (this.routeMap[pathX - 1][pathY] == 0 || count < this.routeMap[pathX - 1][pathY] || this.routeMap[pathX - 1][pathY] == "p") {
    ...
    key: "nextLoopRuttingMap",
    value: function nextLoopRuttingMap(nextPathsX, nextPathsY, count) {
      var pathsX = new Array();
      var pathsY = new Array();

      for (var i = 0; i < nextPathsX.length; i++) {
        // console.log("移動先の座標たち:"+nextPathsX[i]+","+nextPathsY[i]);
        var minX = nextPathsX[i] - 1;
        var maxX = nextPathsX[i] + 1;
        var minY = nextPathsY[i] - 1;
        var maxY = nextPathsY[i] + 1; // ここからルートマッピングを始める。ダイクストラ法を使う
        // 隣接するマスに対して０が入っているか、カウントが今より大きければ、隣接座標に移る。
        // 初期に移動可能なマスの配列を作り、移動先の配列からまた配列を作るかんじ。
        ...
        if (this.loopRuttingFrag) {
          if (minY >= 0) {
            //左
            if (this.routeMap[nextPathsX[i]][nextPathsY[i] - 1] == 0 || count < this.routeMap[nextPathsX[i]][nextPathsY[i] - 1] || this.routeMap[nextPathsX[i]][nextPathsY[i] - 1] == "p") {
          ...
          if (minX >= 0) {
            //上
            if (this.routeMap[nextPathsX[i] - 1][nextPathsY[i]] == 0 || count < this.routeMap[nextPathsX[i] - 1][nextPathsY[i]] || this.routeMap[nextPathsX[i] - 1][nextPathsY[i]] == "p") {
          ...
          var confirmRouteX = new Array();
          var confirmRouteY = new Array();

          var _c = count - 1; // console.log("現在のカウントは？"+c);


          this.saveRoute(_c, 6, 8, confirmRouteX, confirmRouteY); // console.log("確定経路,経路数"+confirmRouteX.length);
    ...
    key: "saveRoute",
    value: function saveRoute(count, i, j, ConfirmRouteX, ConfirmRouteY) {
      // i,jはrouteMapのプレイヤの固定座標
      // SaveRoute()の考え方。
    ...
    key: "AI2",
    value: function AI2(enemys, player, game, direction, textArea) {
      // AI2の考え方
      ...
      for (var i = 0; i < enemys.length; i++) {
        if (enemys[i].exist == true) {
          ...
          && this.whatEnemyCollision(enemys[i], player) == true) {
            // エネミーの最短経路に障害物がないとき 
            console.log("enemyは" + enemys[i].dirX + "," + enemys[i].dirY + "へ移動します"); //エネミーの進行方向を求める。

            var DirectionTravel = this.whatDirectionTravel(enemys[i]);
            console.log("敵" + i + "は" + DirectionTravel); //エネミーの進行方向に進ませ、エネミーのワールドマップ座標を更新する。

            if (DirectionTravel) {
              this.enemyAnimation(DirectionTravel, enemys[i], game);
            }
          } else {
            console.log("エネミーが動かないとき"); // エネミーが動かないときはエネミーのワールド座標を更新しない。

            this.enemyAnimation2(enemys[i].animeDir, enemys[i], game); // this.createEnemyTexture(this.currentArea,0,0,game,enemys);
            //　敵に隣接していれば敵の攻撃

            if (this.whatAdjacent(enemys[i], player)) {
              var dir = this.whatAttackDirection(enemys[i], player);
    ...
    key: "AI",
    value: function AI(enemys, player, game, direction, textArea) {
      // AIの考え方。
      ...
      for (var i = 0; i < enemys.length; i++) {
        if (enemys[i].exist == true) {
          ...
          && this.whatEnemyCollision(enemys[i], player) == true) {
            ...
            this.enemyAnimation("FRONT", enemys[i], game); //　敵に隣接していれば敵の攻撃
      ...
      textArea.hp(player, game); // const endTime = performance.now(); // 終了時間
    ...
    key: "whatAdjacent",
    value: function whatAdjacent(enemy, player) {
      if (enemy.localX == player.localX && enemy.localY == player.localY - 1) {
        //エネミーはプレイヤの左にいる
        return true;
      }

      if (enemy.localX == player.localX - 1 && enemy.localY == player.localY) {
        //上
        return true;
      }
    ...
    key: "whatAttackDirection",
    value: function whatAttackDirection(enemy, player) {
      if (enemy.localX == player.localX && enemy.localY == player.localY - 1) {
        //エネミーはプレイヤの左にいる
        return "LEFT";
      }
    ...
    key: "attack",
    value: function attack(enemy, player, game) {
      var _this = this;

      console.log("敵の攻撃");
      player.hp -= 2;
      var direction = enemy.whatAttackDirection(enemy, player);
      var beforeOffsetX = 0;
      var beforeOffsetY = 0;
      var afterOffsetX = 0;
      var afterOffsetY = 0;
      ...
      game.toriSound.play();
      setTimeout(function () {
        _this.dirMoveAnimetion(enemy.enemyAnime, beforeOffsetX, beforeOffsetY);

        _this.attackEffect(game);

        setTimeout(function () {
          _this.dirMoveAnimetion(enemy.enemyAnime, afterOffsetX, afterOffsetY);
        }, 100);
      }, 25);
    }
  }, {
    key: "attackEffect",
    value: function attackEffect(game) {
      var _loop = function _loop(i) {
        var ellipse = new PIXI.Graphics() // メソッドチェーンで描画するので、;(セミコロン)を付けない   
        ...
        var x = r * Math.cos(a / 180 * Math.PI); // X座標

        var y = r * Math.sin(a / 180 * Math.PI); // Y座標
    ...
    key: "whatEnemyCollision",
    value: function whatEnemyCollision(enemy, player) {
      console.log("this.currentArea[enemy.dirX][enemy.dirY]は？" + this.currentArea[enemy.dirX][enemy.dirY]);

      if (!(enemy.dirX == player.localX && enemy.dirY == player.localY)) {
        //プレイヤ
        if (!(this.currentArea[enemy.dirX][enemy.dirY] == "9")) {
          //壁
          if (!(this.currentArea[enemy.dirX][enemy.dirY] == "enemy1")) {
            //エネミー
            return true;
          }
        }
    ...
    key: "whatDirectionTravel",
    value: function whatDirectionTravel(enemy) {
      if (enemy.dirY < enemy.localY) {
        //左に進む
        return "LEFT";
      }

      if (enemy.dirX < enemy.localX) {
        //上に進む
        return "UP";
      }
    ...
    key: "createEnemyTexture",
    value: function createEnemyTexture(enemyCurrentArea, offsetX, offsetY, game, enemys) {
      // drawから呼ばれる
      game.app.stage.removeChild(this.enemyContainer);
      this.enemyContainer.removeChildren();
      var i = 0;

      for (var mi = 0; mi < enemyCurrentArea.length; mi++) {
        //マップのx1,x2がいくつあるか
        for (var mj = 0; mj < enemyCurrentArea[0].length; mj++) {
          //y0,y1がいくつあるか
    ...
    key: "dirMoveAnimetion",
    value: function dirMoveAnimetion(enemyAnime, offsetX, offsetY) {
      if (offsetX == 48 && offsetY == 0) {
...
/* harmony import */ var _Player__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./Player */ "./resources/js/Player.js");
/* harmony import */ var _itemmap__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./itemmap */ "./resources/js/itemmap.js");
/* harmony import */ var _backgroundMap__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./backgroundMap */ "./resources/js/backgroundMap.js");
/* harmony import */ var _generalMap__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./generalMap */ "./resources/js/generalMap.js");
/* harmony import */ var _viewArea__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ./viewArea */ "./resources/js/viewArea.js");
/* harmony import */ var _enemyMap__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! ./enemyMap */ "./resources/js/enemyMap.js");
/* harmony import */ var _textArea__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! ./textArea */ "./resources/js/textArea.js");
/* harmony import */ var _sceneManeger__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(/*! ./sceneManeger */ "./resources/js/sceneManeger.js");
function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }
...
var Game = /*#__PURE__*/function () {
  ...
  _createClass(Game, [{
    key: "createMapchipSprite",
    value: function createMapchipSprite(Texture, i, j, offsetX, offsetY, zIn) {
      var ele = new PIXI.Sprite(Texture);
    ...
    key: "MapChipKansokujoWeather",
    value: function MapChipKansokujoWeather(kansokujoName, i, j, offsetX, offsetY, zIn, player) {
      var _this = this;
      ...
      var kansokujo_weather = new PIXI.Graphics() //白ぶち
      ...
      var _loop = function _loop(c) {
        ...
        _this.rainContainer.addChild(line); //コンテナにスプライトを入れる
    ...
    key: "MapChipKansokujo",
    value: function MapChipKansokujo(kansokujoName, i, j, offsetX, offsetY, zIn) {
      //枠
      var kansokujo_waku = new PIXI.Graphics() //白ぶち
      ...
      var ele = new PIXI.Text(kansokujoName, {
        fontFamily: 'メイリオ',
        // フォント
        fontSize: fontookisa,
        // 4文字の時は11,3文字の時は15で枠内に収まる
        fill: 0x000000,
        // 文字色
        stroke: 0x000000,
        // アウトラインの色
    ...
    key: "itemPickUp",
    value: function itemPickUp(itemMapArea, x, y, player) {
    ...
    key: "playerMoveEvent",
    value: function playerMoveEvent(px, py, textArea, game) {
      if (this.mapArea[px][py] == "2") {
        this.choiseButtonSound.play();
        this.dungeonMoveFlag = true;
        this.floorChoice = true; // this.sceneDungeon1 = true;
    ...
    key: "playerCollision",
    value: function playerCollision(direction, player, item, background, game, viewArea, enemy, enemys, textArea) {
      ...
      switch (direction) {
        ...
        case LEFT:
          ...
          if (this.mapArea[player.getX()][player.getY() - 1] != '9' && enemy.enemyMap[player.getX()][player.getY() - 1] != "enemy1") {
            // エネミーワールドマップで判定
            // //テスト
            ...
            // });
            // プレイヤのワールド座標の更新
            player.set(player.getX(), player.getY() - 1); // イベント

            this.itemPickUp(item.itemMap, player.getX(), player.getY(), player); // ローカルマップの更新
            ...
            enemy.currentArea = viewArea.getPlayer_Enemys_Area(enemy.enemyMap, player, enemys); // 動かないものをスクロールする。
            ...
            player.walkAnimetion("LEFT", 6, 8, game); // イベント
        ...
        case UP:
          ...
          if (this.mapArea[player.getX() - 1][player.getY()] != '9' && enemy.enemyMap[player.getX() - 1][player.getY()] != "enemy1") {
            // //テスト
            // item.itemMap.forEach(element => {
            //     console.log(element)
            // });
            // プレイヤのワールド座標の更新
            player.set(player.getX() - 1, player.getY()); //イベント
            ...
            player.walkAnimetion("UP", 6, 8, game); //イベント、コンテナに追加した順に重なりが決まるためテキストエリアの表示は最後に持ってくる。
            ...
            enemy.createEnemyTexture(enemy.currentArea, 0, 0, game, enemys); // enemy.AI(enemys,player,game,direction);
          }
        ...
        case RIGHT:
          ...
          if (this.mapArea[player.getX()][player.getY() + 1] != '9' && enemy.enemyMap[player.getX()][player.getY() + 1] != "enemy1") {
            ...
            background.createTexture_from_currentArea(48, 0, game, player, textArea);
            item.createItemMapTexture(item.currentArea, 48, 0, game); // エネミーの行動
            ...
            player.walkAnimetion("RIGHT", 6, 8, game); //イベント
        ...
        case DOWN:
          ...
          if (this.mapArea[player.getX() + 1][player.getY()] != '9' && enemy.enemyMap[player.getX() + 1][player.getY()] != "enemy1") {
            ...
            background.currentArea = viewArea.getPlayerArea(this.mapArea, player);
            item.currentArea = viewArea.getPlayerArea(item.itemMap, player);
            enemy.currentArea = viewArea.getPlayer_Enemys_Area(enemy.enemyMap, player, enemys); //テスト

            enemy.currentArea.forEach(function (element) {
              console.log(element);
            });

            for (var i = 0; i < enemys.length; i++) {
              console.log("エネミーワールド座標" + enemys[i].x + "," + enemys[i].y);
              console.log("エネミーローカル座標:" + enemys[i].localX + "," + enemys[i].localY); // console.log("エネミーの存在チェック:"+enemys[i].exist);            
            ...
            player.walkAnimetion("DOWN", 6, 8, game); // イベント
    ...
    key: "fadeIn",
    value: function fadeIn(container, sprite, player) {
      var _this2 = this;
      ...
      setTimeout(function () {
        _this2.fadeIn(sprite, container, player);
      }, 100);
    }
  }, {
    key: "fadeOut",
    ...
    // }
    //シーン遷移テスト
    value: function fadeOut(container, sprite, player, item, background, game, viewArea, textArea, enemys, enemy) {
      var _this3 = this;
      ...
      setTimeout(function () {
        _this3.fadeOut(container, sprite, player, item, background, game, viewArea, textArea, enemys, enemy);
      }, 100);
    }
  }, {
    key: "draw",
    // メインループ
    value: function draw(game, player, item, background, viewArea, enemy, enemys, textArea) {
      var _this4 = this;

      // 背景の設置
      background.createTexture_from_currentArea(0, 0, game, player, textArea); // アイテムの設置

      item.createItemMapTexture(item.currentArea, 0, 0, game); // 敵の設置,アニメーションの開始

      enemy.createEnemyTexture(enemy.currentArea, 0, 0, game, enemys); // 固定描画(HPと所持金と持ち物)の表示

      textArea.gold(player, game);
      textArea.hp(player, game); // プレイヤの設置とプレイヤアニメーションの開始
      ...
      pushed[ESC] = false;
      pushed[Z] = false;
      this.moveflag = true; // あるキーが押されたときのイベントリスナーを設定
      ...
      this.app.ticker.add(function (delta) {
        ...
        // this.gold(player);
        frameCount += delta; //deltaは大体1秒に60カウントなので10だと1秒に６回の入力を受け付ける

        if (frameCount >= 8) {
          if (_this4.dungeonMoveFlag) {
            // ここでthis.dungeonMoveFlagがtrueなら移動だけ禁止して降りるかどうかの選択メニューをだせるようにする
            //降りるかどうか決定されたとき。
            if (pushed[ENTER]) {
              if (_this4.floorChoice) {
                // 選択肢下る,上る
                _this4.app.stage.removeChild(textArea.kaisouCircle);

                _this4.app.stage.removeChild(textArea.kaisouText);

                _this4.app.stage.removeChild(textArea.choiceTriangle);

                frameCount = 0;
                _this4.dungeonMoveFlag = false;
                _this4.moveflag = true;
                var scene = new _sceneManeger__WEBPACK_IMPORTED_MODULE_7__["default"]();

                if (_this4.sceneDungeon1) {
                  scene.sceneDungeon1(_this4.gameScene1, player.mvCharacterAnime[0], player, item, background, game, viewArea, textArea, enemys, enemy);
                }

                if (_this4.sceneTown) {
                  scene.sceneTown(_this4.gameScene1, player.mvCharacterAnime[0], player, item, background, game, viewArea, textArea, enemys, enemy);
                }

                _this4.stepsSound.play();
              } else if (!_this4.floorChoice) {
                // 選択肢そのまま
                _this4.app.stage.removeChild(textArea.kaisouCircle);
            ...
            if (pushed[LEFT]) {
              // 選択肢。
              if (!_this4.floorChoice) {
                _this4.choiseButtonSound.play();
              }

              _this4.floorChoice = true;
            ...
            if (pushed[RIGHT]) {
              ...
              _this4.floorChoice = false;
              textArea.choiceTriangle.x = textArea.kaisouText.x + 70;
            }
          }

          if (pushed[LEFT] && _this4.moveflag && _this4.dungeonMoveFlag == false) {
            console.log("レフト！"); // ←キーが押されていた場合              

            _this4.moveflag = _this4.playerCollision(LEFT, player, item, background, game, viewArea, enemy, enemys, textArea); //衝突判定等

            frameCount = 0;
            _this4.moveflag = false; // if(player.animeDir!=pushed[LEFT])
          ...
          if (pushed[UP] && _this4.moveflag && _this4.dungeonMoveFlag == false) {
            // ↑キーが押されていた場合
            _this4.playerCollision(UP, player, item, background, game, viewArea, enemy, enemys, textArea); //衝突判定等


            frameCount = 0;
            _this4.moveflag = false;
            setTimeout(function () {
              player.mvCharacterAnime[0].animationSpeed = 0.035; // アニメーション速                
            }, 125);
          }

          if (pushed[RIGHT] && _this4.moveflag && _this4.dungeonMoveFlag == false) {
            // →キーが押されていた場合
            _this4.playerCollision(RIGHT, player, item, background, game, viewArea, enemy, enemys, textArea); //衝突判定等
          ...
          if (pushed[DOWN] && _this4.moveflag && _this4.dungeonMoveFlag == false) {
            // ↓キーが押されていた場合
            _this4.playerCollision(DOWN, player, item, background, game, viewArea, enemy, enemys, textArea); //衝突判定等
          ...
          if (pushed[ESC] && _this4.moveflag && _this4.dungeonMoveFlag == false) {
            console.log("ESC!");
            player.mvCharacterAnime[0].stop();
            frameCount = 0;
          }

          if (pushed[Z] && _this4.moveflag && _this4.dungeonMoveFlag == false) {
            _this4.playerCollision(Z, player, item, background, game, viewArea, enemy, enemys, textArea); //衝突判定等
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "default", function() { return GeneralMap; });
function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }
...
var GeneralMap = /*#__PURE__*/function () {
  function GeneralMap() {
    _classCallCheck(this, GeneralMap);
  }

  _createClass(GeneralMap, [{
    key: "getCSV",
    value: function getCSV(num) {
    ...
    key: "convertCSVtoArray2",
    value: function convertCSVtoArray2(str, game) {
      // 読み込んだCSVデータが文字列として渡される
      var result = {}; // 最終的な二次元配列を入れるための配列
      ...
      var nitizi = tmp[1].split(',')[4] + "年" + tmp[1].split(',')[5] + "月" + tmp[1].split(',')[6] + "日" + tmp[1].split(',')[7] + "時" + tmp[1].split(',')[8] + "分"; // 雨量の更新日時を設定
    ...
    key: "getDate",
    value: function getDate(date, game) {
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "default", function() { return ItemMap; });
function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }
...
var ItemMap = /*#__PURE__*/function () {
  ...
  _createClass(ItemMap, [{
    ...
    key: "setItemMap",
    value: function setItemMap(mapAry) {
      //全体マップをコピーしてアイテムを置き換える
      // 2次元配列の初期化
    ...
    key: "createItemMapTexture",
    value: function createItemMapTexture(itemCurrentArea, offsetX, offsetY, game) {
      for (var mi = 0; mi < itemCurrentArea.length; mi++) {
        //マップのx1,x2がいくつあるか
        for (var mj = 0; mj < itemCurrentArea[0].length; mj++) {
          //y0,y1がいくつあるか
    ...
    value: function deleteItem() {
      for (var y = 0; y < this.itemMap.length; y++) {
        this.itemMap[y] = new Array(this.itemMap[0].length).fill(0);
      }

      for (var mi = 0; mi < this.currentArea.length; mi++) {
        //マップのx1,x2がいくつあるか
        for (var mj = 0; mj < this.currentArea[0].length; mj++) {
          //y0,y1がいくつあるか
...
/* harmony import */ var _Player__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./Player */ "./resources/js/Player.js");
/* harmony import */ var _itemmap__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./itemmap */ "./resources/js/itemmap.js");
/* harmony import */ var _backgroundMap__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./backgroundMap */ "./resources/js/backgroundMap.js");
/* harmony import */ var _generalMap__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ./generalMap */ "./resources/js/generalMap.js");
/* harmony import */ var _viewArea__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! ./viewArea */ "./resources/js/viewArea.js");
/* harmony import */ var _enemyMap__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! ./enemyMap */ "./resources/js/enemyMap.js");
/* harmony import */ var _textArea__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(/*! ./textArea */ "./resources/js/textArea.js");
...
var generalMap = new _generalMap__WEBPACK_IMPORTED_MODULE_4__["default"]();
var promise = [];
...
var item = new _itemmap__WEBPACK_IMPORTED_MODULE_2__["default"]();
var background = new _backgroundMap__WEBPACK_IMPORTED_MODULE_3__["default"]();
var viewArea = new _viewArea__WEBPACK_IMPORTED_MODULE_5__["default"]();
var enemy = new _enemyMap__WEBPACK_IMPORTED_MODULE_6__["default"]();
var enemys = new Array();
var textArea = new _textArea__WEBPACK_IMPORTED_MODULE_7__["default"]();
...
promise.then(function (value) {
  ...
  player.loader.load(function () {
    // 画像の読み込みを待つ
    item.loader.load(function () {
      // 画像の読み込みを待つ
      enemy.loader.load(function () {
        // 画像の読み込みを待つ
        background.loader.load(function () {
          // 画像の読み込みを待つ
          // 全体マップをアイテムマップにコピーしてitemを設置する
          // コピーした配列の中にはpが入っているのでそこを中心としたitemCurrenetAreaを設定する。
          item.setItemMap(value);
          item.currentArea = viewArea.getFirstPlayerArea(item.itemMap, player, "p"); // エネミーの設定
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "default", function() { return SceneManeger; });
/* harmony import */ var _generalMap__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./generalMap */ "./resources/js/generalMap.js");
/* harmony import */ var _enemyMap__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./enemyMap */ "./resources/js/enemyMap.js");
function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }
...
var SceneManeger = /*#__PURE__*/function () {
  function SceneManeger() {
    _classCallCheck(this, SceneManeger);
  }

  _createClass(SceneManeger, [{
    key: "sceneDungeon1",
    value: function sceneDungeon1(container, sprite, player, item, background, game, viewArea, textArea, enemys, enemy) {
      this.fadeOut(container, sprite, player, item, background, game, viewArea, textArea, enemys, enemy);
    }
  }, {
    key: "sceneTown",
    value: function sceneTown(container, sprite, player, item, background, game, viewArea, textArea, enemys, enemy) {
      this.fadeOut(container, sprite, player, item, background, game, viewArea, textArea, enemys, enemy);
    }
  }, {
    key: "fadeOut",
    value: function fadeOut(container, sprite, player, item, background, game, viewArea, textArea, enemys, enemy) {
      ...
      setTimeout(function () {
        _this.fadeOut(container, sprite, player, item, background, game, viewArea, textArea, enemys, enemy);
      }, 100);
    }
  }, {
    key: "fadeIn",
    ...
    key: "LoadTown",
    //fadeIn終わり
    value: function LoadTown(player, item, background, game, viewArea, textArea, enemys, enemy) {
      var _this3 = this;
      ...
      game.gameScene1.removeChildren(); // 新しいマップの読み込み。

      var promise = [];
      var generalMap = new _generalMap__WEBPACK_IMPORTED_MODULE_0__["default"]();
      game.dungeonLevel++;
      promise = generalMap.getCSV(0); // 雨量の読み込み

      var promise2 = [];
      var generalMap2 = new _generalMap__WEBPACK_IMPORTED_MODULE_0__["default"]();
      promise2 = generalMap2.getRainFallCSV(game); // 初期化とか

      player.mvCharacterAnime[0].animationSpeed = 0.035;
      game.dungeonMoveFlag = false; // 初期表示時は階段の上にいないのでfalseにしておく。

      game.sceneDungeon1 = false; // 固定描画を消去
      ...
      promise.then(function (value) {
        promise2.then(function (value2) {
          ...
          for (var key in game.rainfalls) {
            console.log('key:' + key + ' value:' + game.rainfalls[key] + "回数" + c++);
          } //新しいマップからローカル背景エリアを設定。


          background.currentArea = viewArea.getFirstPlayerArea(value, player, "p");
          game.mapArea = value; //今のマップにいるエネミーをすべて削除

          enemy.deleteEnemy(value, enemys); //アイテムを全て削除

          item.setItemMap(game.mapArea);
          item.deleteItem(); // 初期画面を表示、drawの代わり。

          game.playerCollision(DOWN, player, item, background, game, viewArea, enemy, enemys, textArea); // フェードイン

          player.mvCharacterAnime[0].alpha = 0; // フェードインが始まる前にプレイヤを見えなくしておく。

          _this3.fadeIn(player.mvCharacterAnime[0], game.gameScene1, player); // フェードイン終了後に入力を受け付けるようにしてある。
    ...
    key: "LoadDungeon",
    value: function LoadDungeon(player, item, background, game, viewArea, textArea, enemys, enemy) {
      var _this4 = this;
      ...
      var generalMap = new _generalMap__WEBPACK_IMPORTED_MODULE_0__["default"]();
      game.dungeonLevel++;
      promise = generalMap.getCSV(game.dungeonLevel); // 初期化とか
      ...
      promise.then(function (value) {
        ...
        _this4.fadeIn(player.mvCharacterAnime[0], game.gameScene1, player); // フェードイン終了後に入力を受け付けるようにしてある。
...
var TextArea = /*#__PURE__*/function () {
  ...
  _createClass(TextArea, [{
    key: "weatherDate",
    value: function weatherDate(game) {
      var rainfallsDateText = new PIXI.Text(game.rainfallsDate + "更新", {
      ...
      rainfallsDateText.zIndex = 40; // text.text = '0123\n456789';   // テキストの書き換え

      game.app.stage.addChild(rainfallsDateText);
    }
  }, {
    key: "gold",
    value: function gold(player, game) {
      // なぜかキーボード押すとテキストエリアが消える。
    ...
    value: function hp(player, game) {
      ...
      .drawRect(101, 21, this.hp1, 8).endFill();
      this.hpText3.zIndex = 20;
      game.app.stage.addChild(this.hpText3); // this.choiceTriangle = new PIXI.Graphics()
    ...
    key: "kaisouMove",
    value: function kaisouMove(game, text) {
      var _this = this;

      this.kaisouCircle = new PIXI.Graphics().beginFill(0x0000FF, 0.3) //色,透明度
      .lineStyle(2, 0x000000).drawRoundedRect(game.app.screen.width / 2 - 140, game.app.screen.height / 2 - 40, 280, 80, 10).endFill();
      ...
      this.kaisouText.zIndex = 100; //x位置,y位置,xスケール,yスケール,回転,xスキュー,yスキュー,xピポッド,yピポッド
      // this.kaisouText.setTransform(this.kaisouText.x, this.kaisouText.y, 1, 1,  0,  0,0.2,  1,1 );

      game.app.stage.addChild(this.kaisouText);
      this.choiceTriangle = new PIXI.Graphics().beginFill(0xFFFFFF, 0.8) // 第二引数で透明度を指定できる
      .drawPolygon([// 頂点を配列で渡す [x1,y1,x2,y2,....]
      0, 0, //左上
      0, 16, //左下
      8, 8 //右真ん中
      ...
      game.app.stage.addChild(this.choiceTriangle); //選択肢のアイコン▷の点滅

      setTimeout(function () {
        _this.choiceTriangle.alpha = 0;
        setTimeout(function () {
          _this.choiceTriangle.alpha = 1;
        }, 500);
      }, 500);
    }
  }, {
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "default", function() { return ViewArea; });
function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }
...
var ViewArea = /*#__PURE__*/function () {
  ...
  _createClass(ViewArea, [{
    key: "getFirstPlayerArea",
    // 第一引数mapAryはgeneralMapとitemMapが渡される。第二引数でマップのpの位置を探してプレイヤエリアを導き出す
    value: function getFirstPlayerArea(mapAry, player, target) {
      // 2次元配列の初期化
      var currentArea = new Array(ViewWidth); // 移動のための2マス文多めにとる
      ...
      for (var mi = 0; mi < mapAry.length; mi++) {
        ...
        for (var mj = 0; mj < mapAry[0].length; mj++) {
          ...
          if (mapAry[mi][mj] == target) {
            ...
            for (var i = mi - 6; i < mi + 7; i++) {
              //mapのキャラを中心としたcurrentAreaの左上から
              for (var j = mj - 8; j < mj + 9; j++) {
                //currentAreaの右下の座標まで(オフセット２マス分含む)
    ...
    key: "getPlayerArea",
    value: function getPlayerArea(mapAry, player) {
      // 2次元配列の初期化
      var currentArea = new Array(ViewWidth); //移動のための2マス文多めにとる
      ...
      for (var mi = 0; mi < mapAry.length; mi++) {
        ...
        for (var mj = 0; mj < mapAry[0].length; mj++) {
          //y0,y1がいくつあるか
          if (mi == player.getX() && mj == player.getY()) {
            //map上のプレイヤーの位置を求める
            // console.log("マップの中でのp座標:"+mi+","+mj);
            for (var i = mi - 6; i < mi + 7; i++) {
              //mapのキャラを中心としたcurrentAreaの左上から
    ...
    key: "getPlayer_Enemys_Area",
    value: function getPlayer_Enemys_Area(mapAry, player, enemys) {
      // 2次元配列の初期化
      var currentArea = new Array(ViewWidth); //移動のための2マス文多めにとる
      ...
      for (var i1 = 0; i1 < enemys.length; i1++) {
          ...
          if (mi == player.getX() && mj == player.getY()) {
            ...
            for (var i = mi - 6; i < mi + 7; i++) {
              ...
              for (var j = mj - 8; j < mj + 9; j++) {
                ...
                for (var _i = 0; _i < enemys.length; _i++) {
                  if (enemys[_i].x >= mi - 6 && enemys[_i].x < mi + 7) {
                    if (enemys[_i].y >= mj - 8 && enemys[_i].y < mj + 9) {
                      if (enemys[_i].x == i && enemys[_i].y == j) {
                        enemys[_i].localX = xx;
                        enemys[_i].localY = yy;
                        enemys[_i].exist = true;
                      }
                    }
                  }
                }
...
module.exports = __webpack_require__(/*! C:\Program Files\Git\mysite\task_portfolio\resources\js\main.js */"./resources/js/main.js");
